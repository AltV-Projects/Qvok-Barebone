...
resourceGenerator.availableResources.forEach(resource => {
    ...
    if (pkg.isGameResource) {
        const hasServer = resourceGenerator.hasFolder(resource, pkg.serverFolder || 'server');
        const hasClient = resourceGenerator.hasFolder(resource, pkg.clientFolder || 'client');
